...
export default function Edit() {
  ...
  const getHotDeal = () => {
    axiosInstance.get(`hot-deals/${id}`).then(response => {
      setHotDealForm(response.data)
    }).finally(() => {
  ...
  const editHotDeal = () => {
    setLoading(true)
    axiosInstance.put(`hot-deals/${id}`, hotDealForm).then(response => {
      navigate('/hot-deals')
    }).catch(err => {
  ...
  return (
    <div>
      ...
      >
        <div>
          <h2 className='text-xl'>{t("editHotDeal")}</h2>
          <div className='flex flex-col gap-1'>
            <Input value={hotDealForm.titleEn} label={t('englishName')} inputValue={(value: string) => setHotDealForm({ ...hotDealForm, titleEn: value })} width="lg:w-1/3 w-96" disabled={loading} errorMessage={t(errorData.titleEn)} />
            <Input value={hotDealForm.titleAr} label={t('arabicName')} inputValue={(value: string) => setHotDealForm({ ...hotDealForm, titleAr: value })} width="lg:w-1/3 w-96" disabled={loading} errorMessage={t(errorData.titleAr)} />
            ...
            <button className={`w-full h-12 bg-primaryColor mt-7 text-white rounded-md`} onClick={editHotDeal}>
