...
export default function SingleShopItem(props: SingleShopItemProps) {
    ...
    return (
        <tr>
            ...
            <td className="whitespace-nowrap py-4 pl-4 pr-3 text-xs sm:pl-6">
                <div>
                    <div className="font-medium text-gray-900">{t(props.value.itemType)}</div>
                </div>
            </td>
            <EditAndDelete route={`/edit-shop-item/${props.value._id}`} deleteFunc={() => props.deleteFunc(props.value._id)} />
        </tr>
